(()=>{
  ...
  const normalizeLearnerFooter=()=>{
    if(document.body.classList.contains('admin-body')||document.body.classList.contains('instructor-app')||document.body.classList.contains('checkout-page')||document.querySelector('.player-layout,.certificate-wrap'))return;
    const footer=document.querySelector('footer,.portal-footer');
    if(!footer)return;
