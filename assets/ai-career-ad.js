(()=>{
'use strict';
const PLACEMENT='homepage_ai_career_learn_more_ad';
const PUBLIC_URL='/api/advertisements/homepage-ai-career-learn-more-ad';
const ADMIN_URL='/api/admin/manage/advertisement-placements/homepage-ai-career-learn-more-ad';
...
async function setupAdmin(){
  ...
  const install=async()=>{
    ...
    if(!panel||$('#aiCareerPlacementForm'))return false;

    const card=document.createElement('div');
    card.className='admin-card admin-placement-card';
    card.innerHTML=`
      <strong>Homepage — AI Career “Learn more” Ad</strong>
      <p style="margin:6px 0 0;color:#667085">Choose the ad that should trigger when a visitor clicks “Learn more” in the “Reimagine your career in the AI era” section.</p>
      <form id="aiCareerPlacementForm" class="admin-placement-grid">
        <label>Selected advertisement<select id="aiCareerAdvertisement"><option value="">None / disabled</option></select></label>
        <button class="admin-primary" type="submit">Save placement</button>
      </form>
      <p id="aiCareerPlacementMessage" class="admin-muted" style="margin:10px 0 0"></p>`;

    const certification=$('#certificationCtaPlacementForm',panel)?.closest('.admin-placement-card');
    const longbar=$('#longbarPlacementForm',panel)?.closest('.admin-placement-card');
    const anchor=certification||longbar;
    if(anchor)anchor.insertAdjacentElement('afterend',card);
    else panel.querySelector('.admin-heading')?.insertAdjacentElement('afterend',card);

    const select=$('#aiCareerAdvertisement');
    const msg=$('#aiCareerPlacementMessage');
    ...
    const refresh=async()=>{
      try{
        ...
        msg.textContent=selected?`Currently selected: ${selected.name}.`:'No advertisement is currently assigned to this Learn more button.';
      }catch(err){msg.textContent=err?.message||'Could not load AI Career ad placement.'}
    };
    ...
    card.querySelector('form').addEventListener('submit',async e=>{
      ...
      try{
        ...
        await refresh();
        msg.textContent=id?'AI Career Learn more placement saved successfully.':'AI Career Learn more placement disabled.';
      }catch(err){msg.textContent=err?.data?.errors?Object.values(err.data.errors).flat().join(' '):(err?.message||'Could not save AI Career ad placement.')}
    });
...
async function setupHomepage(){
  ...
  const cta=document.querySelector('.ai-banner .light-btn');
  if(!cta)return;
