...
function injectCss(){
  ...
  const l=document.createElement('link');l.rel='stylesheet';l.href='/assets/course-access-links.css?rev=20260810-course-link-v1';l.dataset.courseAccessCss='1';document.head.appendChild(l);
}
...
function accessAction(item){
  const href=validUrl(item.course_link);
  return href
...
function renderLearning(){
  ...
  list.innerHTML=access.map(item=>{
    const c=catalog.get(item.slug)||{},href=validUrl(item.course_link),image=c.image||fallbackImage;
...
function renderOrderLinks(){
  ...
  root.querySelectorAll('.order-card2').forEach(card=>{
    ...
    const pill=card.querySelector('.status-pill');if(pill)pill.textContent=orderStatusText(order.status||'completed');
    ...
    const rows=ids.map(slug=>{
      ...
      const href=item?validUrl(item.course_link):null;
      if(href)return `<div><span>${esc(title)}</span><a href="${esc(href)}" target="_blank" rel="noopener noreferrer">Open course ↗</a></div>`;
      return `<div><span>${esc(title)}</span><small>${order.status==='pending'?'Available after payment verification':'Course link not set'}</small></div>`;
    }).join('');
    if(!rows)return;
