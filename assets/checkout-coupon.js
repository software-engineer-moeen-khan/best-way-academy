...
function subtotalFromPage(){const raw=$('#checkoutSubtotal')?.textContent||'0';return Number(raw.replace(/[^0-9.-]/g,''))||0}
function setMessage(text,type=''){const el=$('#checkoutCouponMessage');if(!el)return;el.textContent=text||'';el.classList.remove('success','error');if(type)el.classList.add(type)}
function setSubmitMessage(text,type=''){const el=$('#checkoutMessage');if(!el)return;el.textContent=text||'';el.classList.remove('success','error');if(type)el.classList.add(type)}
function emit(name,detail={}){document.dispatchEvent(new CustomEvent(name,{detail}))}
function resetQuote(){
  const subtotal=subtotalFromPage(),row=$('#checkoutDiscountRow');if(row)row.hidden=true;
  ...
  appliedCode='';remove('bwa_pending_discount');emit('bwa:coupon-reset',{subtotal,total:subtotal});
}
...
async function waitForOrder(){
  for(let i=0;i<100;i++){
    ...
    const items=$('#checkoutItems .checkout-item');
    if(!loading&&(items||requestedSlugs().length===0))return true;
...
async function applyCoupon(code,quiet=false){
  ...
  code=String(code||input?.value||'').trim().toUpperCase();if(input)input.value=code;
  if(!code){resetQuote();setMessage('Enter a coupon code.','error');return}
  const slugs=requestedSlugs();if(!slugs.length){resetQuote();setMessage('Your cart is empty.','error');return}
  if(button){button.disabled=true;button.textContent='Checking…'}setSubmitMessage('');if(!quiet)setMessage('Validating coupon with the server…');
  try{
    ...
    appliedCode=String(coupon.code||code).toUpperCase();write('bwa_pending_discount',{code:appliedCode});
    ...
    resetQuote();const text=err?.data?.errors?Object.values(err.data.errors).flat().join(' '):(err?.message||'Coupon could not be applied.');setMessage(text,'error');
  }finally{if(button){button.disabled=false;button.textContent=appliedCode?'Applied':'Apply'}}
}
async function init(){
  if(location.pathname!=='/checkout'&&!document.body.classList.contains('checkout-page'))return;
  const input=$('#checkoutCouponCode'),button=$('#checkoutApplyCoupon');if(!input||!button)return;
  button.disabled=true;await waitForOrder();button.disabled=false;
  input.addEventListener('input',()=>{input.value=input.value.toUpperCase();if(appliedCode&&input.value.trim().toUpperCase()!==appliedCode){resetQuote();setMessage('Coupon changed. Press Apply to validate the new code.')}else if(!input.value.trim()){resetQuote();setMessage('')}});
  ...
  const pending=read('bwa_pending_discount',null)?.code;if(pending){input.value=String(pending).toUpperCase();await applyCoupon(pending,true)}
}
