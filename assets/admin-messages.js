...
function installUi(){
  ...
  if(main&&!main.querySelector('[data-panel="messages"]')){
    ...
    section.innerHTML=`
      <div class="admin-heading"><div><p class="eyebrow">INBOX</p><h1>Messages</h1><p>Learner support and public support messages in one place.</p></div><button id="adminMessagesRefresh" class="admin-outline" type="button">Refresh messages</button></div>
      <div class="admin-stat-grid admin-message-stats"><article><span>Support messages</span><strong id="adminDirectMessageCount">0</strong><small>Direct learner support threads</small></article><article><span>Contact support</span><strong id="adminContactMessageCount">0</strong><small>Website support requests</small></article><article><span>Incoming</span><strong id="adminIncomingMessageCount">0</strong><small>Messages needing attention</small></article></div>
      <div class="admin-toolbar"><input id="adminMessageSearch" type="search" placeholder="Search sender, email, subject or message…"><select id="adminMessageType"><option value="">All messages</option><option value="support">Support thread</option><option value="contact">Contact support</option></select></div>
  ...
  if(!$('#adminMessageReplyDialog')){
    document.body.insertAdjacentHTML('beforeend',`<dialog id="adminMessageReplyDialog" class="admin-dialog"><form id="adminMessageReplyForm"><div class="dialog-head"><div><p class="eyebrow">REPLY</p><h2 id="adminMessageReplyTitle">Reply to support message</h2></div><button type="button" data-admin-message-close>×</button></div><input type="hidden" name="user_id"><input type="hidden" name="channel" value="support"><label>Subject<input name="subject" maxlength="180" placeholder="Support reply from Best Way Academy"></label><label>Message<textarea name="body" rows="7" required maxlength="5000" placeholder="Write your reply…"></textarea></label><div class="dialog-actions"><button type="button" class="admin-outline" data-admin-message-close>Cancel</button><button type="submit" class="admin-primary">Send reply</button></div></form></dialog>`);
  }

  if(!$('#adminMessageCenterStyle')){
    const style=document.createElement('style');style.id='adminMessageCenterStyle';
    style.textContent='.admin-message-stats{grid-template-columns:repeat(3,minmax(0,1fr));margin-bottom:18px}.admin-message-preview{max-width:620px;white-space:normal;line-height:1.45}.admin-message-preview b{display:block;margin-bottom:4px}.admin-message-preview small{display:block;margin-top:5px;color:#6b7280}.message-channel{display:inline-flex;padding:4px 8px;border-radius:999px;background:#ecfdf5;font-size:12px;font-weight:800}.message-channel.contact{background:#fff7ed}.admin-message-direction{font-size:12px;font-weight:700}.admin-message-direction.out{color:#64748b}@media(max-width:1024px){.admin-message-stats{grid-template-columns:1fr 1fr}}@media(max-width:640px){.admin-message-stats{grid-template-columns:1fr}}';
...
function rows(){
  const direct=(inbox.messages||[]).filter(x=>x.channel==='support').map(x=>({...x,type:'support'}));
...
function render(){
  const direct=(inbox.messages||[]).filter(x=>x.channel==='support');
  $('#adminDirectMessageCount')&&($('#adminDirectMessageCount').textContent=String(direct.length));
  $('#adminContactMessageCount')&&($('#adminContactMessageCount').textContent=String((inbox.support_requests||[]).length));
  $('#adminIncomingMessageCount')&&($('#adminIncomingMessageCount').textContent=String(direct.filter(x=>!x.from_admin).length+(inbox.support_requests||[]).filter(x=>!['resolved','closed'].includes(x.status)).length));
  ...
  body.innerHTML=list.map(x=>{
    const channel=x.type==='contact'?'Contact support':'Support';
    ...
    const actions=x.type==='contact'?`<a class="admin-small admin-link" href="mailto:${encodeURIComponent(x.sender_email||'')}">Email</a>`:`<button class="admin-small primary" data-message-reply="${x.user_id}" data-message-name="${safe(x.sender_name||'User')}">Reply</button>`;
...
function openReply(button){
  ...
  form.reset();form.elements.user_id.value=button.dataset.messageReply;form.elements.channel.value='support';
  form.elements.subject.value='Support message reply';
  $('#adminMessageReplyTitle').textContent=`Reply to ${button.dataset.messageName||'user'}`;
  dialog.showModal();
}

async function sendReply(form){
  const user=form.elements.user_id.value,subject=form.elements.subject.value.trim(),body=form.elements.body.value.trim();
  if(!body)return;
  ...
  try{const b=await backend();await b.api(`/api/admin/manage/messages/${user}/reply`,{method:'POST',body:JSON.stringify({channel:'support',subject:subject||null,body})});$('#adminMessageReplyDialog').close();await load()}
