...
class SadaPayPaymentController extends Controller
{
    ...
    private const NUMBER_KEY = 'sadapay_account_number';
    ...
    public function config(): JsonResponse
    {
        ...
        return response()->json([
            'enabled' => $hasQr || $accountNumber !== '',
            'qr_url' => $hasQr ? '/api/payment/sadapay/qr' : null,
    ...
    public function qr()
    {
        ...
        abort_unless($path, 404, 'SadaPay payment QR is not configured.');
    ...
    public function updateConfig(Request $request): JsonResponse
    {
        ...
        if ($number !== '') {
            abort_unless((bool) preg_match('/^[A-Za-z0-9+() .\-]{5,60}$/', $number), 422, 'Enter a valid SadaPay account/mobile number.');
        }

        DB::table('platform_settings')->updateOrInsert(
            ['key' => self::NUMBER_KEY],
            [
                'value' => json_encode($number, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE),
                'updated_at' => now(),
                'created_at' => now(),
            ]
        );
    ...
    public function uploadQr(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'ok' => true,
            'enabled' => true,
            'qr_url' => '/api/payment/sadapay/qr?rev=' . now()->timestamp,
    ...
    public function removeQr(Request $request): JsonResponse
    {
        abort_unless($request->user()?->role === 'admin', 403);
        if (Schema::hasTable('payment_assets')) {
            DB::table('payment_assets')->where('key', self::QR_KEY)->delete();
        }
        $this->deleteLegacyQr();

        return response()->json([
            'ok' => true,
            'enabled' => $this->accountNumber() !== '',
            'account_number' => $this->accountNumber() ?: null,
        ]);
    }
    ...
    public function submit(Request $request): JsonResponse
    {
        ...
        $user = $request->user();
        $already = DB::table('enrollments as e')
            ->join('courses as c', 'c.id', '=', 'e.course_id')
            ->where('e.user_id', $user->id)
            ->whereIn('c.slug', $slugs)
            ->pluck('c.slug');
        ...
        $result = DB::transaction(function () use ($courses, $user, $data, $reference) {
            ...
            if ($total > 0) {
                abort_unless($this->hasQr() || $this->accountNumber() !== '', 422, 'SadaPay payment details are not configured yet.');
                abort_if($reference === '', 422, 'Enter your SadaPay transaction/reference ID after sending payment.');

                $escapedReference = str_replace(['%', '_'], ['\\%', '\\_'], $reference);
                $duplicate = DB::table('orders')
                    ->where('user_id', $user->id)
                    ->where('payment_method', 'sadapay')
                    ->whereIn('status', ['pending', 'completed'])
                    ->where('metadata', 'like', '%' . $escapedReference . '%')
                    ->exists();
            ...
            $meta = [
                ...
                'payment_channel' => $total === 0 ? 'coupon_free' : 'sadapay_manual',
                'payment_reference' => $reference !== '' ? $reference : null,
                'payment_submitted_at' => $total > 0 ? now()->toIso8601String() : null,
        ...
        return response()->json([
            ...
            'payment_pending' => $result['status'] === 'pending',
        ], 201);
    }

    private function accountNumber(): string
    {
        $value = DB::table('platform_settings')->where('key', self::NUMBER_KEY)->value('value');
        if ($value === null) {
            return '';
        }
        if (!is_string($value)) {
        ...
        return trim((string) (json_last_error() === JSON_ERROR_NONE ? $decoded : $value));
    }

    private function hasQr(): bool
    {
        if (Schema::hasTable('payment_assets') && DB::table('payment_assets')->where('key', self::QR_KEY)->exists()) {
            return true;
        }
    ...
    {
        foreach (['jpg', 'jpeg', 'png', 'webp'] as $ext) {
            Storage::disk('public')->delete('payments/sadapay-qr.' . $ext);
        }
    }
}
