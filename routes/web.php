...
use App\Http\Controllers\CertificationAdController;
...
Route::get('/api/advertisements/homepage-certifications-cta-ad',[CertificationAdController::class,'show'])->middleware('throttle:120,1');
Route::middleware('auth')->group(function(){
    ...
    Route::put('/api/admin/manage/advertisement-placements/homepage-certifications-cta-ad',[CertificationAdController::class,'assign'])->middleware('throttle:30,1');
...
$serveHtml=function(string $file){
    ...
    $html=preg_replace('/(\/assets\/[A-Za-z0-9_.\/-]+\.(?:css|js))(?:\?v[^"\']*)?/','$1?v=20260810-45',$html);

    if(!str_contains($html,'portal-polish.css')){
        $html=str_ireplace('</head>','  <link rel="stylesheet" href="/assets/portal-polish.css?v=20260810-45">'.PHP_EOL.'</head>',$html);
    }
    if(!str_contains($html,'responsive-final.css')){
        $html=str_ireplace('</head>','  <link rel="stylesheet" href="/assets/responsive-final.css?v=20260810-45">'.PHP_EOL.'</head>',$html);
    }
    if(!str_contains($html,'responsive-unified.css')){
        $html=str_ireplace('</head>','  <link rel="stylesheet" data-bwa-responsive-unified="1" href="/assets/responsive-unified.css?v=20260810-45">'.PHP_EOL.'</head>',$html);
    }
    if(!str_contains($html,'nav-cleanup.css')){
        $html=str_ireplace('</head>','  <link rel="stylesheet" href="/assets/nav-cleanup.css?v=20260810-45">'.PHP_EOL.'</head>',$html);
    }
    if(!str_contains($html,'backend-sync.js')){
        $html=str_ireplace('</body>','<script src="/assets/backend-sync.js?v=20260810-45"></script>'.PHP_EOL.'</body>',$html);
    }
    if(!str_contains($html,'backend-actions.js')){
        $html=str_ireplace('</body>','<script src="/assets/backend-actions.js?v=20260810-45"></script>'.PHP_EOL.'</body>',$html);
    }
    if(!str_contains($html,'admin-backend.js')){
        $html=str_ireplace('</body>','<script src="/assets/admin-backend.js?v=20260810-45"></script>'.PHP_EOL.'</body>',$html);
    }
    if(!str_contains($html,'instructor-backend.js')){
        $html=str_ireplace('</body>','<script src="/assets/instructor-backend.js?v=20260810-45"></script>'.PHP_EOL.'</body>',$html);
    }
    if(!str_contains($html,'clean-route-fixes.js')){
        $html=str_ireplace('</body>','<script src="/assets/clean-route-fixes.js?v=20260810-45"></script>'.PHP_EOL.'</body>',$html);
    }
    if(!str_contains($html,'portal-polish.js')){
        $html=str_ireplace('</body>','<script src="/assets/portal-polish.js?v=20260810-45"></script>'.PHP_EOL.'</body>',$html);
    }
    if(!str_contains($html,'content-cleanup.js')){
        $html=str_ireplace('</body>','<script src="/assets/content-cleanup.js?v=20260810-45"></script>'.PHP_EOL.'</body>',$html);
    }
    if(!str_contains($html,'certification-ad.js')){
        $html=str_ireplace('</body>','<script src="/assets/certification-ad.js?v=20260812-1"></script>'.PHP_EOL.'</body>',$html);
    }
